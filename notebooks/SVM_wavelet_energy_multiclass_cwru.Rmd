---
title: "Multiclass classification using SVM on wavelet packet energy features"
...
data_wav_energy = read.csv(paste("./feature_matrix/48k",
                          "/feature_matrix_wav_energy8_48k_2048_load_1.csv",
                          sep = ""), header = T)
...
* Regarding hyper parameters of SVM, readers are encouraged to try different cost values. Trying a higher cost allows for more misclassified points while training. For this case and the previous case, slight improvements can be obtained in test accuracy by using a higher cost value. We have set the upper value of cost to be 200 for no good reason. 

* Finally, more details about wavelet packet features and the ways (with codes) to compute those will appear in a future post. For the time being, readers can download and use the feature matrix already created by us.
